...
// Advertise advertises images and files to the given routing.Router.
...
func Advertise(ctx context.Context, r routing.Router, containerdStore containerd.Store, filesChan <-chan string) {
	...
	for {
		...
		case <-ticker:
			...
			err := advertiseAll(ctx, l, containerdStore, r)
			if err != nil {
		...
		case ref := <-eventCh:
			...
			_, err := advertiseRef(ctx, l, containerdStore, r, ref)
			if err != nil {
...
// advertiseAll advertises all references in the containerd store using the provided logger and router.
// It returns an error if any error occurs during the advertisement process.
func advertiseAll(ctx context.Context, l zerolog.Logger, containerdStore containerd.Store, router routing.Router) error {
	...
	for _, ref := range refs {
		_, err := advertiseRef(ctx, l, containerdStore, router, ref)
		if err != nil {
...
// advertiseRef advertises the given containerd reference by extracting its digest and tags,
...
func advertiseRef(ctx context.Context, l zerolog.Logger, containerdStore containerd.Store, router routing.Router, ref containerd.Reference) (int, error) {
	keys := []string{}
