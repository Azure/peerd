...
func TestPartialContentResponseInP2PMode(t *testing.T) {
	...
	s, err := store.NewMockStore(context.Background(), tests.NewMockRouter(make(map[string][]string)))
	if err != nil {
		t.Fatal(err)
	}

	h := New(context.Background(), s)
...
func TestNotFoundInP2PMode(t *testing.T) {
	...
	store.PrefetchWorkers = 0 // turn off prefetching
	s, err := store.NewFilesStore(context.Background(), tests.NewMockRouter(make(map[string][]string)))
	if err != nil {
		t.Fatal(err)
	}
